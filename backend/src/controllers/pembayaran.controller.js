...
// Otomatis "masuk ke akun orang tua" karena siswa sudah punya relasi orangTuaId;
// endpoint GET / di atas memfilter berdasarkan relasi itu untuk role ORANGTUA.
...
async function verifikasi(req, res) {
  try {
    const { id } = req.params;
    const { status, catatan, metode } = req.body; // status: LUNAS | DITOLAK
