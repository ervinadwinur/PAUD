...
async function register(req, res) {
  try {
    ...
    if (!username || !email || !password || !nama) return error(res, "Username, email, password, dan nama wajib diisi", 400);
    const hashed = await bcrypt.hash(password, 10);
    const user = await prisma.user.create({
      data: { username, email, password: hashed, role: "ORANGTUA", orangTua: { create: { nama, noTelepon, alamat } } },
