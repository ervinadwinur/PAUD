...
const Absensi = () => {
  ...
  useEffect(() => { siswaService.getAll().then((res) => { const anak = res.data.data || []; setAnakList(anak); setSelectedAnak(anak[0]?.id || null); }); }, []);
  useEffect(() => { if (selectedAnak) absensiService.getAll({ siswaId: selectedAnak }).then((res) => setDataAbsensi(res.data.data || [])); }, [selectedAnak]);
